...
|------|------------|-------|--------|
| 1 | лексер + AST: ключевые слова, `&`, `ast.Op`, `BoolLiteral` | ~60 строк + ~60 тестов | [x] 0.1.17 |
| 2 | парсер: приоритеты, `NOT`, `TRUE`/`FALSE`, `: BOOL` | ~40 строк + ~120 тестов | [x] 0.1.18 |
| 3 | sema: BOOL как тип, логические операции, `NOT`, сравнение | ~120 строк + ~200 тестов | [x] 0.1.19 |
| 4 | codegen: четыре таблицы, `cOps`, `NOT`, булев литерал | ~30 строк | [x] 0.1.20 |
...
| 2026-09-20 | 1 | 0.1.17 | Лексер: семь ключевых слов (`BOOL`, `TRUE`, `FALSE`, `AND`, `OR`, `XOR`, `NOT`), токен `AMP` для `&` (в `tokenNames` вписан руками) и ветка разбора в `NextToken`. AST: операции `AND`/`OR`/`XOR` и унарная `NOT` в `ast.Op` (в `opNames` — словами), узел `BoolLiteral` с `String()` → `Bool(TRUE)`/`Bool(FALSE)`. Тесты лексера: четыре кейса в `TestAllTokens` (литералы и `: BOOL`, логические операторы, `&` через пробел и вплотную, префиксы `AND1`/`NOTx`/`ORDER`/`BOOLEAN`/`TRUEX`/`FALSEHOOD`/`XORS` остаются `IDENT`), строка регистра в `TestKeywordsCaseInsensitive`, четвёртая строка с `&` в `TestColumns` | Отклонений от плана нет. Промежуточное состояние проверено руками: `x : BOOL;` → `expected type name, got BOOL`, `a AND b` и `a & b` → `expected ;, got AND` / `got &` (`&`, а не `UNKNOWN` — ловушка `tokenNames` обойдена). Golden-эталоны `.ast` и `.c` не тронуты, `go test ./...` зелено |
| 2026-09-20 | 2 | 0.1.18 | Парсер: `prec` перенумерована по IEC (`OR` 1 < `XOR` 2 < `AND`/`&` 3 < `= <>` 4 < отношения 5 < `+ -` 6 < `* /` 7), `binOps` дополнена (`AMP` → та же `ast.AND`), унарные операторы сведены в таблицу `unaryOps` (`MINUS` → `NEG`, `NOT` → `NOT`) по образцу `binOps` — `parseUnary` берёт оператор из неё и рекурсивен для обоих; `TRUE`/`FALSE` → `BoolLiteral` в `parsePrimary`; `BOOL` в `parseType`. Тесты: 16 кейсов в `TestExpressionStructure` (таблица плана целиком плюс `a = b < c AND d`, `a & b OR c & d`, `NOT a = b` → `(NOT a) = b`, `NOT -x`, `NOT` слабее `.`/вызова), `: BOOL` с `TRUE`/`false` в `TestVarBlocks`, новый `TestBoolLiteral` (значение и позиция литерала, BOOL как вход и тип возврата FUNCTION, логическое условие `IF`), шесть негативных в `TestParseErrors` (`x := AND b`, `x := & b` — пиннит `&` в `tokenNames`, `x := NOT;`, `x := a AND;`, `TRUE := x`, `a AND b;`). Golden `.ast`: `go test ./src/parser && git diff --stat src/parser/testdata` — diff пуст, `-update` не запускался | Ветка `NOT` сделана не отдельным `if`, а таблицей `unaryOps` — симметрично `binOps`. Поправлен устаревший комментарий у `ast.UnaryExpr` («Op всегда NEG»). Промежуточное состояние проверено руками через CLI: `x : BOOL;` и `FUNCTION F : BOOL` → sema `unknown type "BOOL"`; `i AND j` и `i & j` над INT → codegen `unsupported operator AND`; `i := NOT i` → codegen `unsupported unary operator NOT`; `IF a > 1 AND b < 2` → sema `operator "AND" is not applicable to BOOL` (AND пока считается арифметикой). Нюанс для этапа 3: `i := TRUE;` проходит sema **молча** — `infer` для незнакомого узла возвращает `Invalid`, что гасит каскад, и литерал падает лишь в codegen (`unsupported expression *ast.BoolLiteral`); ветка `*ast.BoolLiteral` в `infer` из плана этапа 3 это закрывает. `-dump-ast` идёт после sema, поэтому дерево с `: BOOL` через CLI пока не посмотреть — структура закреплена тестами |
| 2026-09-20 | 3 | 0.1.19 | Sema: `BOOL` в `builtinType`; `isScalar` (INT/REAL/BOOL — единственная точка перечисления) для `scalarOnly` и `returnKind`; `BoolLiteral` → `Bool` в `infer`, `hintOf`, `exprTok`; унарный `NOT` в `infer` (операнд с контекстом BOOL, обязан быть BOOL: `NOT requires a BOOL operand, got INT`) и в `hintOf` → `Bool`; `inferBinary` разведена по классам: `isLogical` → новая `inferLogical` (оба операнда в контексте BOOL, подсказка соседа не используется: `operator "AND" requires BOOL operands, got INT and INT`), сравнение — BOOL только для `=`/`<>`, порядковые над BOOL через `isOrdering` — `cannot order BOOL values (operands of "<" are BOOL and INT; BOOL is compared only with = and <>)`, арифметика — прежнее `is not applicable to BOOL`; `hintOf` у логической операции — `Bool`; условие IF — `IF condition must be BOOL, got INT`. Тесты: `TestBool` (17 кейсов: позитив по всем контекстам плана; негативы — число в BOOL и BOOL в число, BOOL в арифметике, логические над INT/REAL включая `&` и одни литералы, `NOT` над не-BOOL, приоритет `NOT x > 1`, порядковые сравнения, ловушка `a < b < c`, равенство BOOL с числом, IF не-BOOL включая экземпляр ФБ, FOR по BOOL, функция и ФБ с BOOL, отсутствие каскада) плюс проверка, что ни одна ошибка не подсказывает `INT_TO_REAL`; `TestInfo` — `b1 AND b2`, `TRUE`, `NOT b2` в side-table | Формулировки: шесть подстрок «IF condition must be BOOL (a comparison)» в `TestTypes`/`TestReal` → без «(a comparison)»; кейс `TestTypes` с `(i > 1) = (i < 3)` по решению 6 стал законным — заменён на `(i > 1) < (i < 3)` с `cannot order BOOL values`. Сверх плана: `isScalar` вместо дублирования `k == Int \|\| k == Real` в двух местах; `inferLogical` и `isOrdering` — отдельные функции ради читаемости `inferBinary`; сообщение о порядковом сравнении дополнено типами операндов и подсказкой про `=`/`<>`, чтобы в ловушке `a < b < c` было видно, откуда взялся BOOL. Golden `.ast`/`.c` не тронуты, `go test ./...` зелено. Промежуточное состояние через CLI: все негативы из «Верификации» дают ошибки sema с `line:col`; позитивная программа проходит sema и падает в codegen `no C mapping for type "BOOL"` — этап 4 |
| 2026-09-21 | 4 | 0.1.20 | Codegen: `cTypes["BOOL"] = "_Bool"`, `cZeros["BOOL"] = "0"`, `cFormats["BOOL"] = "%d"` (в `cWideTypes` не добавлен — как REAL); `cOps` дополнена `AND` → `&&`, `OR` → `\|\|`, `XOR` → `!=` (обоснование решения 3 — комментарием у таблицы); унарные операции сведены в таблицу `cUnaryOps` (`NEG` → `-`, `NOT` → `!`) по образцу `cOps`/парсерной `unaryOps` — жёсткое `if e.Op != ast.NEG` в `expr` снято; `BoolLiteral` → `1`/`0`. Регрессия: `go test ./src/codegen -update && git diff --stat src/codegen/testdata` — diff пуст, все двенадцать `.c` побайтово прежние. Проверено руками: позитивная цепочка из «Верификации» (`b=1 n=5`), плюс программа с `XOR`/`OR`/`&`/`NOT NOT`, BOOL-функцией и ФБ-защёлкой (`q := (q OR set) AND NOT rst`) — `gcc -std=c99 -Wall -Wextra` без предупреждений, значения по расчёту; семь негативов дают ошибки sema с `line:col` | Сверх плана: кейсы `BOOL`/`Bool` в `TestZeroLiteral` (`literal_test.go`). Комментарий у `cTypes` дополнен «почему `_Bool`», у `cFormats` — «BOOL печатается 0/1». Golden `.ast`/`.c` не тронуты, `go test ./...` зелено. Корпусный пример `bool_all.st` и эталоны — этап 5 |
